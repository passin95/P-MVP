...
public class AppDelegateImpl implements AppDelegate,IArms{
    ...
    public AppDelegateImpl(Context context) {
        ...
        // 遍历之前获得的集合，执行每一个 ConfigModule 实现类的方法。
        for (ModuleConfig module : mModules) {

            // 将框架外部，开发者实现的 Application 的生命周期回调 (AppLifecycles) 存入 mAppLifecycles 集合 (此时还未注册回调)。
            module.injectAppLifecycle(context, mAppLifecycles);

            // 将框架外，开发者实现的 Activity 的生命周期回调 (ActivityLifecycleCallbacks) 存入 mActivityLifecycles 集合 (此时还未注册回调)。
    ...
    public void onCreate(Application application) {
        ...
        // 开发者可在框架外部对 ComponentCallbacks2 进行监听，
...
//                case TRIM_MEMORY_COMPLETE:
         ...
         * {@link ComponentCallbacks2#onTrimMemory(int)} 的回调是在 API 14 才被加进来的，对于老的版本，你可以使用
         * {@link ComponentCallbacks2#onLowMemory} 方法来进行兼容。
