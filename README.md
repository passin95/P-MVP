...
## 优势
- 将所有的 activity 、fragment、dialog都看成一个page，以page为单位使用定义P和M层的@Scope（生命周期），相对来说很自由的复用P层和M层。
...
- 底层基础库作为一个项目的根基所在因此异常重要，同时为了发现问题能够及时的处理，因此不推荐远程依赖的方式使用（Watch便好），再加上个人时间规划原因，因此不提供远程依赖的方式（但会不定时更新自己的见解）。
